...
export class ProcedureService {
  private activeRuns: Map<string, ProcedureRun> = new Map();
   ...
   */
  async startProcedure(procedureId: string, context?: Record<string, any>): Promise<ProcedureRun> {
    // Get the procedure
   ...
   */
  async resumeProcedure(runId: string): Promise<ProcedureRun | null> {
   ...
   */
  async getCurrentStep(runId: string): Promise<ProcedureStep | null> {
    const run = this.activeRuns.get(runId);
   ...
   */
  async executeStep(runId: string, userInput?: any): Promise<StepResult> {
    const run = this.activeRuns.get(runId);
    if (!run || run.status !== 'active') {
    ...
    const result = await stepExecutor.executeStepWithRetry(
      step,
      context,
      run.stepResponses
    );
   ...
   */
  async checkProcedureRequirement(context: ProcedureContext): Promise<EnforcementResult> {
   ...
   */
  private findActiveRunForProcedure(procedureId: string): ProcedureRun | null {
