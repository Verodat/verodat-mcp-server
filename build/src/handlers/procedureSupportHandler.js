...
export class ProcedureSupportHandler {
     ...
     */
    static async handleQuizAnswer(args) {
        try {
            ...
            if (Array.isArray(quizStep.correctAnswer)) {
                isCorrect = Array.isArray(answer)
                    ...
                    : quizStep.correctAnswer.includes(answer);
            }
            else {
     ...
     */
    static async handleApprovalCheck(args) {
        try {
            ...
            const approvalStep = currentStep;
     ...
     */
    static async handleInformationAck(args) {
        try {
     ...
     */
    static async handleWaitContinue(args) {
        try {
     ...
     */
    static getToolForStepType(stepType) {
        switch (stepType) {
            case 'quiz':
