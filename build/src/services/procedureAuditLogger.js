...
export class ProcedureAuditLogger {
     ...
     */
    initialize(toolHandlers, workspaceId, accountId) {
     ...
     */
    async logProcedureStart(run, procedure) {
        if (procedure) {
     ...
     */
    async logProcedureComplete(run) {
        await this.logEvent({
            procedure_id: run.procedureId,
            run_id: run.runId,
            event_type: 'complete',
     ...
     */
    async logProcedureFail(run, error) {
        await this.logEvent({
            procedure_id: run.procedureId,
            run_id: run.runId,
            event_type: 'fail',
     ...
     */
    async logStepComplete(run, stepId, stepName) {
        await this.logEvent({
            procedure_id: run.procedureId,
            run_id: run.runId,
     ...
     */
    async logStepFail(run, stepId, stepName, error) {
        await this.logEvent({
            procedure_id: run.procedureId,
            run_id: run.runId,
     ...
     */
    async logToolAuthorized(runId, toolName, procedureId) {
        await this.logEvent({
    ...
    async ensureAuditDataset() {
        ...
        try {
            ...
            const auditDataset = datasets.find((ds) => ds.name === 'Procedure Audit Log');
            if (auditDataset) {
