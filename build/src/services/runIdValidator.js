...
import { isWriteOperation } from '../config/operationTypes.js';
export class RunIdValidator {
     ...
     */
    async validateRunIdForTool(runId, toolName, args) {
        try {
            ...
            if (isWriteOperation(toolName)) {
                const writeValidation = await this.validateWriteOperation(toolName, procedure, activeRun, args);
     ...
     */
    extractGovernedTools(procedure) {
        const tools = new Set();
        // Add tools from triggers
        if (procedure.triggers?.tools) {
            procedure.triggers.tools.forEach((tool) => tools.add(tool));
        }
        ...
        if (procedure.steps) {
            procedure.steps.forEach((step) => {
                ...
                if (step.validation?.allowedTools) {
                    step.validation.allowedTools.forEach((tool) => tools.add(tool));
                }
            });
        }
     ...
     */
    getStepAllowedTools(procedure, stepIndex) {
     ...
     */
    async validateWriteOperation(toolName, procedure, activeRun, args) {
     ...
     */
    isProcedureManagementTool(toolName) {
        return [
            'start-procedure',
            'list-procedures',
            'resume-procedure',
