...
export class ProcedureLoader {
     ...
     */
    async loadProcedures(force = false) {
    ...
    async findApplicableProcedures(context) {
        ...
        for (const entry of this.cache.values()) {
            ...
            if (!procedure.isActive)
                continue;
            const now = new Date();
            if (procedure.effectiveFrom && new Date(procedure.effectiveFrom) > now)
                continue;
            if (procedure.effectiveTo && new Date(procedure.effectiveTo) < now)
                continue;
            // Check triggers
            let matches = false;
