...
function getAuthToken(toolName) {
...
async function makeAPIRequest(url, method, headers, body) {
    try {
        ...
        return { data: JSON.stringify(responseData, null, 2) };
    }
    catch (error) {
        return {
            data: null,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "create-dataset") {
            const { accountId, workspaceId, name, description, targetFields, } = CreateDatasetArgumentsSchema.parse(args);
            ...
            const { accountId, workspaceId, offset = 0, max = 9999, filter } = GetDatasetsArgumentsSchema.parse(args);
            ...
            const { data, error } = await makeAPIRequest(url, "GET", headers);
            if (error) {
                return {
                    content: [
                        {
            ...
            const { accountId, workspaceId, datasetId, offset = 0, max = 1000, filter } = GetDatasetOutputArgumentsSchema.parse(args);
            ...
            const { accountId } = GetWorkspacesArgumentsSchema.parse(args);
            ...
            const { accountId, workspaceId, } = GetAIContextArgumentsSchema.parse(args);
            ...
            const { accountId, workspaceId, query } = ExecuteAIQueryArgumentsSchema.parse(args);
            ...
            const { accountId, workspaceId, datasetId, offset, max, filter, sort } = GetDatasetTargetFieldsArgumentsSchema.parse(args);
