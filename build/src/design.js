...
import { DesignToolHandler } from './handlers/designToolHandler.js';
...
async function handleTestMode() {
    ...
    if (args.length < 3 || args[0] !== 'call') {
        console.error('Usage: node design.js call <tool-name> <json-args>');
        process.exit(1);
    }
    ...
    const server = new Server({ name: 'verodat-test', version: '1.0.0' }, { capabilities: { tools: {} } });
    // Create tool handlers
    const allToolHandlers = new ToolHandlers(API_BASE_URL, API_KEY);
    const designToolHandler = new DesignToolHandler(server, allToolHandlers, ServerType.DESIGN);
    // Initialize procedures (will be skipped for DESIGN server)
    await designToolHandler.initializeProcedures();
    // Register tools
    designToolHandler.registerTools();
    // Try to execute the tool
    try {
        const handler = designToolHandler['toolHandlerMap'].get(toolName);
        if (!handler) {
            throw new Error(`Unknown tool: ${toolName}`);
        }
        // Check procedure requirement first (should always return false for DESIGN)
        const procedureCheck = await designToolHandler['checkProcedureRequirement'](toolName, toolArgs);
...
async function main() {
    ...
    try {
        ...
        const allToolHandlers = new ToolHandlers(API_BASE_URL, API_KEY || '');
        // Initialize DESIGN category handler with server type
        const designToolHandler = new DesignToolHandler(server, allToolHandlers, ServerType.DESIGN);
        // Register the tools
        designToolHandler.registerTools();
        // Error handling
