...
async function handleTestMode() {
    ...
    if (args.length < 3 || args[0] !== 'call') {
        console.error('Usage: node manage.js call <tool-name> <json-args>');
        process.exit(1);
    }
    ...
    const manageToolHandler = new ManageToolHandler(server, allToolHandlers, ServerType.MANAGE);
    // Initialize procedures to enable enforcement
    await manageToolHandler.initializeProcedures();
    // Register tools
    manageToolHandler.registerTools();
    // Try to execute the tool
    try {
        const handler = manageToolHandler['toolHandlerMap'].get(toolName);
        if (!handler) {
            throw new Error(`Unknown tool: ${toolName}`);
        }
        // Check procedure requirement first
        const procedureCheck = await manageToolHandler['checkProcedureRequirement'](toolName, toolArgs);
...
async function main() {
    ...
    try {
        ...
        const allToolHandlers = new ToolHandlers(API_BASE_URL, API_KEY || '');
        // Initialize MANAGE category handler with server type
        const manageToolHandler = new ManageToolHandler(server, allToolHandlers, ServerType.MANAGE);
        // Register the tools
        manageToolHandler.registerTools();
        // Error handling
