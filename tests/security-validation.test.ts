...
describe('Security Validation - RunId Hijacking Prevention', () => {
  ...
  describe('RunId Validation', () => {
    it('should prevent using PROC-EXPORT-DATA runId for create-dataset', async () => {
      ...
      sandbox.stub(procedureService, 'getActiveRun').returns(exportProcRun as any);
      sandbox.stub(procedureLoader, 'getProcedure').resolves(exportProcedure as any);
    ...
    it('should allow using correct runId for authorized tools', async () => {
      ...
      // Mock service methods
      sandbox.stub(procedureService, 'getActiveRun').returns(writeProcRun as any);
      sandbox.stub(procedureLoader, 'getProcedure').resolves(writeProcedure as any);
    ...
    it('should enforce step-level tool restrictions', async () => {
      ...
      // Mock service methods
      sandbox.stub(procedureService, 'getActiveRun').returns(multiStepRun as any);
      sandbox.stub(procedureLoader, 'getProcedure').resolves(multiStepProcedure as any);

      // Try to use upload-dataset-rows in step 2 (should fail)
      const invalidStepValidation = await runIdValidator.validateRunIdForTool(
        'run-multistep-111',
        'upload-dataset-rows',
        {}
      );
      ...
      const validStepValidation = await runIdValidator.validateRunIdForTool(
        'run-multistep-111',
        'create-dataset',
        {}
      );
  ...
  describe('Audit Logging', () => {
    it('should log security violations to audit service', async () => {
      ...
      // Attempt hijack
      await runIdValidator.validateRunIdForTool(
        'run-audit-test',
        'delete-dataset',  // Attempting dangerous operation
        { datasetId: 123 }
      );
  ...
  describe('Wildcard Pattern Expansion', () => {
    it('should correctly expand tool patterns with wildcards', async () => {
      ...
      sandbox.stub(procedureService, 'getActiveRun').returns(procRun as any);
      sandbox.stub(procedureLoader, 'getProcedure').resolves(wildcardProcedure as any);

      // Should allow get-datasets (matches get-*)
      const getValidation = await runIdValidator.validateRunIdForTool(
        'run-pattern-222',
        'get-datasets',
        {}
      );
      ...
      const createValidation = await runIdValidator.validateRunIdForTool(
        'run-pattern-222',
        'create-dataset',
        {}
      );
      ...
      const uploadValidation = await runIdValidator.validateRunIdForTool(
        'run-pattern-222',
        'upload-dataset-rows',
        {}
      );
